...
async function questionnaireFunction(q_id, format) {
    let tokenValue;
    try{
    ...
    try {
        const response = await axios.get('http://localhost:9103/intelliq_api/questionnaire/' + q_id, { "data": {
            "token": tokenValue
          }
        });
        if (format == "json"){ 
            ...
            const fields = ['questionnaireID', 'questionnaireTitle', 'keywords'];
            
            const questionnaire_data = json2csv(response.data, {fields});

            console.log(questionnaire_data);

            //const fields2 = ['qID', 'qtext', 'type', 'required'];

            const question_data = json2csv(response.data.questions);
