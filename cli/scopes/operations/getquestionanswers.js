...
async function getQuestionAnswers(Q_id, qst_id, format) {
  let tokenValue;
    try{
    ...
    try {
      const response = await axios.get('http://localhost:9103/intelliq_api/getquestionanswers/' + Q_id + '/' + qst_id, {"data": {
            "token": tokenValue
          }
          });
      if (format == "json"){ 
